...
type App struct {
	...
	IgnoreFile         string         `json:"ignore_text"`
	WorkPath           string
	IgnoreRules        string
}
...
func loadConfig() {
	...
	text, err := ioutil.ReadFile(configFilePath)
	if err != nil {
		log.Fatal(err)
	}
	...
	for idx, app := range config.Apps {
		if !filepath.IsAbs(app.BuildPath) {
			config.Apps[idx].BuildPath, err = filepath.Abs(filepath.Join(filepath.Dir(configFilePath), app.BuildPath))
			if err != nil {
				log.Fatal(err)
			}
		}
		...
		loadIgnore(&config.Apps[idx])
	}

	log.Printf("load config ok. path=%s\n", configFilePath)
}

func loadIgnore(app *App) {
	if len(app.IgnoreFile) == 0 {
		app.IgnoreFile = "ignore.txt"
	}
	if !filepath.IsAbs(app.IgnoreFile) {
		app.IgnoreFile = filepath.Join(filepath.Dir(configFilePath), app.IgnoreFile)
	}
	log.Printf("load ignore file %s\n", app.IgnoreFile)
	_, err := os.Stat(ignoreFilePath)
	if err != nil {
		app.IgnoreRules = ""
		return
	}
	text, err := ioutil.ReadFile(app.IgnoreFile)
	if err != nil {
	...
	if rText != "" {
		...
		ignores = strings.Replace(ignores, ".", "\\.", -1)
		app.IgnoreRules = ignores
	}
}

func extractEmbedFiles(efs embed.FS, src, dst string) {
	entries, _ := efs.ReadDir(src)
	for _, entry := range entries {
		from := src + "/" + entry.Name()
		to := dst + "/" + entry.Name()
		if entry.IsDir() {
			extractEmbedFiles(efs, from, to)
		} else {
			src, _ := efs.Open(from)
			defer src.Close()
...
func createVersionConfig(app App) {
	...
	if strings.HasSuffix(version, ".h") {
		if !filepath.IsAbs(version) {
			version = filepath.Join(filepath.Dir(configFilePath), version)
		}
		log.Printf("use version file %s\n", version)
...
func main() {
	...
	if len(config.Compiler) == 0 {
		isccPath := os.TempDir() + "\\iscc_14"
		os.Mkdir(isccPath, 0700)
		config.Compiler = isccPath + "\\iscc.exe"
		log.Printf("use iscc path %s\n", isccPath)
		extractEmbedFiles(iscc, "iscc", isccPath)
	}
